...
NOTE: Since `1.2.x` release, this sub-chapter only applies for versions below `1.2.x`.
...
Next, run `tailor update` in all `ocp-config` folders from all the repositories
to bring OCP resources (such as DC or BC) into sync. Review the diff produced by
Tailor carefully, especially around changes to PVCs. To find each `ocp-config` folder,
you can use e.g. `find . -name ocp-config -not -path "./ods-configuration*"` on Unix.

==== Shared images

After all OCP resources have been updated, you need to start a build for all build configs
in the `cd` and `shared-services` namespaces to create new images.
...
Then, push the vanilla branch or tag of the shared library to your
...
Finally, in your Rundeck instance:
* Update/Sync all jobs from this release
* Update `quickstarts/airflow_cluster` job to get proper values for `-h`, `-r` and `-oc` flags.
* Run `verify global rundeck settings` job
